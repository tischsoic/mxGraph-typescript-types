...
declare class mxText extends mxShape {
	...
	constructor(value: string, bounds: mxRectangle, align: string, valign: string, color: string,
		family: string,	size: number, fontStyle, spacing: number, spacingTop: number, spacingRight: number,
		spacingBottom: number, spacingLeft: number, horizontal: boolean, background: string, border: string,
		wrap: boolean, clipped: boolean, overflow: overflow, labelPadding: number, textDirection: text_direction);
	// {
	// 	mxShape.call(this);
	// 	this.value = value;
	// 	this.bounds = bounds;
	 ...
	 */
	textWidthPadding: number; //(document.documentMode == 8 && !mxClient.IS_EM) ? 4 : 3
	 ...
	 */
	getAutoDirection(): text_direction;
	// {
	 ...
	 * Updates the HTML node(s) to reflect the latest bounds and scale.
	 */
	updateSize(node, enableWrap): void;
	// {
