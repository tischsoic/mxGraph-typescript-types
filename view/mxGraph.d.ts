declare module "mxgraphAllClasses" {
	interface mxGraphConstructor {
		new (container?: HTMLElement, model?: mxGraphModel, renderHint?: string, stylesheet?: mxStylesheet): mxGraph;
	}
 ...
 */
// TO DO
//declare class mxGraph exteds mxEventSource {
 ...
 */
	 ...
	 */
	pageFormat: any; //mxConstants.PAGE_FORMAT_A4_PORTRAIT;
	 ...
	 */
	defaultLoopStyle: any; // mxEdgeStyle.Loop;
	 ...
	 */
	swimlaneIndicatorColorAttribute: any; //mxConstants.STYLE_FILLCOLOR;
	 ...
	 */
	warningImage: mxImage; //new mxImage(mxClient.imageBasePath + '/warning'+
	 ...
	 */
	alreadyConnectedResource: string; //(mxClient.language != 'none') ? 'alreadyConnected' : '';
	 ...
	 */
	containsValidationErrorsResource: string; //(mxClient.language != 'none') ? 'containsValidationErrors' : '';
	 ...
	 */
	// TO DO
	// in types from github the type is: createHandlers(container); why???
	createHandlers(): void;
	// {
	 ...
	 */
	setSelectionModel(selectionModel: mxGraphSelectionModel): void; // check if it is really GRAPHSelectionModel
	// {
	 ...
	 */
	// TO DO
	// return type an types for parameters
	 ...
	 */
	// TO DO
	// create mxChange and check if it is mxCell
	getRemovedCellsForChanges(changes): mxCell[]; // getRemovedCellsForChanges(changes: mxChange[]): mxCell[];
	// {
	// 	var result = [];
	 ...
	 */
	// TO DO
	// create mxChange interface???
	processChange(change): void;
	// {
	// 	// Resets the view settings, removes all cells and clears
	// 	// the selection if the root changes.
	// 	if (change instanceof mxRootChange)
	 ...
	 */
    // TO DO
	// check exact type of evt HERE AND IN OTHER PLACES!!!
	startEditingAtCell(cell: mxCell, evt: any): void;
	// {
	 ...
	 */
	getEditingValue(cell: mxCell, evt: any): string;
	// {
	...
	// TO DO
	// is 'value' a string?
	labelChanged(cell: mxCell, value: string, evt: any): mxCell;
	// {
    ...
    // TO DO
	// evt
	dblClick(evt: any, cell: mxCell): void;
	// {
	// 	var mxe = new mxEventObject(mxEvent.DOUBLE_CLICK, 'event', evt, 'cell', cell);
	// 	this.fireEvent(mxe);
    ...
    // TO DO
	// types
	postProcessCellStyle(style: any): void;
	// {
	// 	if (style != null)
	// 	{
	 ...
	 */
    // TO DO
	// maybe specify more exact return type. Is it possible?
	toggleCellStyles(key: string, defaultValue?: boolean, cells?: mxCell[]): number;
	// {
	 ...
	 * the selection cells.
	 */
	setCellStyleFlags(key: string, flag: number, value: boolean | null, cells: mxCell[]): void;
	// {
	// 	cells = cells || this.getSelectionCells();
	 ...
	 */
	// TO DO
	// params; cells can be null, should I state this in TypeScript explicitly
	alignCells(align: any, cells?: mxCell[], param?: any): mxCell[];
	// {
	// 	if (cells == null)
	// 	{
	 ...
	 */
	groupCells(group: mxCell, border, cells: mxCell[]): mxCell;
	// {
	// 	if (cells == null)
	// 	{
	// 		cells = mxUtils.sortCells(this.getSelectionCells(), true);
	 ...
	 */
	getBoundsForGroup(group, children, border): any;
	// {
	 ...
	 */
	// TO DO
	// Nie jestem pewien zwracanego typu
	 ...
	 * Default is false.
	 */
	insertVertex(parent: mxCell, id: string, value: object | string,
	 ...
	 */
	createVertex(parent: mxCell, id: string, value: object,
		x: number, y: number, width: number, height: number, style: string, relative: boolean): mxCell;
	// {
	 ...
	 * style - Optional string that defines the cell style.
	 */
	insertEdge(parent: mxCell, id: string, value: object | string, source: mxCell, target: mxCell, style?: string): mxCell;
	// {
	 ...
	 */
	addEdge(edge: mxCell, parent: mxCell, source: mxCell, target: mxCell, index): mxCell;
	// {
	 ...
	 * target - Optional <mxCell> that represents the target terminal.
	 */
	addCell(cell: mxCell, parent: mxCell, index, source: mxCell, target: mxCell): mxCell;
	// {
	 ...
	 */
	addCells(cells: mxCell[], parent: mxCell, index, source: mxCell, target: mxCell): mxCell[];
	// {
	// 	if (parent == null)
	// 	{
	 ...
	 */
	cellsAdded(cells, parent, index, source, target, absolute, constrain): void;
	// {
	 ...
	 */
	splitEdge(edge: mxCell, cells: mxCell[], newEdge: mxCell, dx: number, dy: number): mxCell;
	// {
	// 	dx = dx || 0;
	// 	dy = dy || 0;
	...
	// TO DO
	// evt type
	foldCells(collapse: boolean, recurse: boolean, cells: mxCell[], checkFoldable: boolean, evt: any): mxCell[];
	// {
	...
	// TO DO
	// evt, mapping
	importCells(cells: mxCell[], dx: number, dy: number, target: mxCell, evt: any, mapping: any): mxCell[];
	// {	
	...
	// TO DO
	// evt, mapping
	moveCells(cells: mxCell[], dx: number, dy: number, clone?: boolean, target?: mxCell, evt?: any, mapping?: any): mxCell[];
	// {
	// 	dx = (dx != null) ? dx : 0;
	// 	dy = (dy != null) ? dy : 0;
	 ...
	 */
	constrainChild(cell: mxCell, sizeFirst: false): void;
	// {
	 ...
	 */
	getOutlineConstraint(point, terminalState, me): mxConnectionConstraint | null;
	// {
	 ...
	 */
	getTranslateForRoot(cell: mxCell): null;
	// {
	// 	return null;
	// };
	 ...
	 */
	getTerminalForPort(cell: mxCell, source): mxCell;
	// {
	 ...
	 */
	// TO DO
	// should return type be null? HERE AND IN OTHER PLACES 
	getChildOffsetForCell(cell: mxCell): null;
	// {
	// 	return null;
	// };
	...
	// TO DO
	// return type
	getCellBounds(cell: mxCell, includeEdges?: boolean, includeDescendants?: boolean): any;
	// {
	// 	var cells = [cell];
	 ...
	 */
	getBoundingBoxFromGeometry(cells: mxCell[], includeEdges: boolean): any;
	// {
	 ...
	 */
	isGridEnabledEvent(evt): boolean;
	// {
	 ...
	 * Returns true if the given mouse event should be aligned to the grid.
	 */
	isConstrainedEvent(evt): boolean;
	// {
	 ...
	 */
	isIgnoreTerminalEvent(evt): boolean;
	// {
	// 	return false;
	// };
	 ...
	 * target - <mxCell> that represents the target terminal.
	 */
	getEdgeValidationError(edge: mxCell, source: mxCell, target: mxCell): string | null;
	// {
	 ...
	 * target - <mxCell> that represents the target terminal.
	 */
	validateEdge(edge: mxCell, source: mxCell, target: mxCell): void;
	// {
	// 	return null;
	// };
	 ...
	 */
	validateGraph(cell?: mxCell, context?): string | null;
	// {
	// 	cell = (cell != null) ? cell : this.model.getRoot();
	 ...
	 * context - Object that represents the global validation state.
	 */
	validateCell(cell: mxCell, context): null;
	// {
	// 	return null;
	// };
	 ...
	 */
	getFoldingImage(state): mxImage | null;
	// {
	 ...
	 */
	getTooltip(state: mxCellState, node, x: number, y: number): any;
	// {
	// 	var tip = null;
	 ...
	 */
	getCursorForMouseEvent(me: mxMouseEvent): any;
	// {
	 ...
	 */
	getCursorForCell(cell: mxCell): null;
	// {
	// 	return null;
	// };
	 ...
	 */
	getImage(state: mxCellState): any;
	// {
	 ...
	 * returned.
	 */
	getVerticalAlign(state: mxCellState): any;
	// {
	 ...
	 * returned.
	 */
	getIndicatorColor(state: mxCellState): any;
	// {
	 ...
	 * returned.
	 */
	getIndicatorGradientColor(state: mxCellState): any;
	// {
	 ...
	 */
	setInvokesStopCellEditing(value): void;
	// {
	 ...
	 */
	setEnterStopsCellEditing(value): void;
	// {
	 ...
	 */
	setVertexLabelsMovable(value): void;
	// {
	 ...
	 */
	setEdgeLabelsMovable(value): void;
	// {
	 ...
	 */
	setCellsDisconnectable(value): void;
	// {
	 ...
	 */
	isConnectable(connectable): boolean;
	// {
	 ...
	 */
	setExtendParentsOnAdd(value): void;
	// {
	 ...
	 * value - New boolean value for <extendParentsOnAdd>.
	 */
	setExtendParentsOnMove(value): void;
	// {
	 ...
	 */
	setConstrainChildren(value): void;
	// {
	 ...
	 */
	setConstrainRelativeChildren(value): void;
	// {
	 ...
	 */
	setAllowNegativeCoordinates(value): void;
	// {
	 ...
	 * Returns the cells which are movable in the given array of cells.
	 */
	getFoldableCells(cells: mxCell[], collapse): mxCell[];
	// {
	// 	return this.model.filterCells(cells, mxUtils.bind(this, function(cell)
	// 	{
	 ...
	 * evt - Mouseevent that triggered the invocation.
	 */
	isValidDropTarget(cell: mxCell, cells: mxCell[], evt): boolean;
	// {
	 ...
	 * evt - Mouseevent that triggered the invocation.
	 */
	isSplitTarget(target: mxCell, cells: mxCell[], evt): boolean;
	// {
	 ...
	 */
	getDropTarget(cells: mxCell[], evt, cell: mxCell, clone?: boolean): mxCell;
	// {
	 ...
	 */
	getSwimlaneAt (x, y, parent: mxCell): mxCell;
	// {
	 ...
	 */
	getCellAt(x: number, y: number, parent: mxCell, vertices?: boolean, edges?: boolean, ignoreFn?: (cell: mxCell, x: number, y: number) => boolean): mxCell;
	// {
	 ...
	 * y - Y-coordinate of the location to be checked.
	 */
	intersects(state: mxCellState, x, y): mxCell;
	// {
	// 	if (state != null)
	// 	{
	// 		var pts = state.absolutePoints;
	 ...
	 */
	hitsSwimlaneContent(swimlane: mxCell, x, y): boolean;
	// {
	 ...
	 */
	// TO DO
	// Above, in description should be 'sources' not 'source'
	 ...
	 * directed -
	 */
	getEdgesBetween(source, target, directed): mxCell[];
	// {
	...
	// TO DO
	// check if return type is correct
	getPointForEvent(evt, addOffset?: boolean): mxPoint;
	// {
	 ...
	 */
	getCells(x, y, width, height, parent: mxCell, result?): mxCell[];
	// {
	 ...
	 */
	// TO DO
	// check if all variables after should be otional
	getCellsBeyond(x0, y0, parent?: mxCell, rightHalfpane?: boolean, bottomHalfpane?: boolean): mxCell[];
	// {
	// 	var result = [];
	 ...
	 */
	traverse(vertex: mxCell, directed?: boolean, func?, edge?: mxCell, visited?): void;
	// {
	 ...
	 */
	selectCellForEvent(cell: mxCell, evt?): void;
	// {
	 ...
	 * evt - Optional mouseevent that triggered the selection.
	 */
	selectCellsForEvent(cells: mxCell[], evt?): void;
	// {
	// 	if (this.isToggleEvent(evt))
	// 	{
	 ...
	 */
	createHandler(state: mxCellState): any;
	// {
	// 	var result = null;
	 ...
	 * state - <mxCellState> to create the handler for.
	 */
	createEdgeHandler(state: mxCellState, edgeStyle): mxEdgeHandler;
	// {
	// 	var result = null;
	 ...
	 */
	updateMouseEvent(me: mxMouseEvent, evtName): mxMouseEvent;
	// {
	 ...
	 */
	isEventIgnored(evtName, me, sender): boolean;
	// {
	 ...
	 */
	isSyntheticEventIgnored(evtName, me, sender): boolean;
	// {
	// 	var result = false;
	// 	var mouseEvent = mxEvent.isMouseEvent(me.getEvent());
	 ...
	 */
	isEventSourceIgnored(evtName, me): boolean;
	// {
	 ...
	 */
	fireMouseEvent(evtName, me, sender?): void;
	// {
	 ...
	 */
	consumeMouseEvent(evtName, me, sender): void;
	// {
	 ...
	 */
	fireGestureEvent(evt, cell?: mxCell): void;
	// {
