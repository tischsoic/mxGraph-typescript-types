...
export class mxPrintPreview {
	constructor(graph: mxGraph, scale: number, pageFormat: mxRectangle, border?: number, x0?: number, y0?: number, 
		borderColor?: string, title?: string, pageSelector?: boolean);
	// {
	// 	this.graph = graph;
	 ...
	 */
	pageFormat: mxRectangle | null; //null
	 ...
	 */
	appendGraph(graph: mxGraph, scale: number, x0: number, y0: number, forcePageBreaks?: boolean, keepOpen?: boolean);
	// {
	// 	this.graph = graph;
	// 	this.scale = (scale != null) ? scale : 1 / graph.pageScale;
	// 	this.x0 = x0;
	 ...
	 * this is specified then no HEAD tag, CSS and BODY tag will be written.
	 */
	open(css: string, targetWindow, forcePageBreaks?: boolean, keepOpen?: boolean);
	// {
	 ...
	 */
	renderPage(w: number, h: number, dx: number, dy: number, content: (arg: null | HTMLDivElement) => any, pageNumber: number): HTMLDivElement;
	// {
	// 	var doc = this.wnd.document;
